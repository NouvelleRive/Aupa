...
export default function RecettesPage() {
  ...
  const getPrixProduitFournisseur = (ingredientId: string): number => {
    ...
    if (pfs.length > 0) {
      ...
      return plusRecent.prix / ((plusRecent as any).nbKg || 1) / plusRecent.rendement / ((plusRecent as any).nbPieces || 1);
    }
  ...
  const calculerCout = () => {
    ...
    const coutNoms = nomIngredients.reduce((total, n) => {
      ...
      if (pfs.length > 0) {
        const plusRecent = pfs.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime())[0];
        return total + (plusRecent.prix / ((plusRecent as any).nbKg || 1) / plusRecent.rendement / ((plusRecent as any).nbPieces || 1)) * n.grammage;
      }
      return total;
    }, 0);
                    ...
                    const cout = (r.ingredients || []).reduce((total: number, i: any) => {
                      if (i.ingredientId || i.ingredientIds?.length > 0) {
                        ...
                        for (const id of ids) {
                          ...
                          if (pfs.length > 0) {
                            const plusRecent = pfs.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime())[0];
                            prices.push(plusRecent.prix / ((plusRecent as any).nbKg || 1) / plusRecent.rendement / ((plusRecent as any).nbPieces || 1));
                          } else {
                            const direct = produitsFournisseurs.find(pf => pf.id === id);
                            if (direct) prices.push(direct.prix / ((direct as any).nbKg || 1) / direct.rendement / ((direct as any).nbPieces || 1));
                          }
                        }
                      ...
                      if (i.nomIngredient) {
                        ...
                        if (pfs.length > 0) {
                          const plusRecent = pfs.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime())[0];
                          return total + (plusRecent.prix / ((plusRecent as any).nbKg || 1) / plusRecent.rendement / ((plusRecent as any).nbPieces || 1)) * i.grammage;
                        }
                      }
                      return total;
                    }, 0);
