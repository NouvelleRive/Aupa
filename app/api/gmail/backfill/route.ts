...
// ============================================================================
// Réutilise les mêmes helpers que sync/route.ts
// ============================================================================
...
type RapportPopina = {
  ...
  caTTC: number; caHT: number; couverts: number; commandes: number;
  debutService: string | null; finService: string | null;
...
// ============================================================================
// BACKFILL — pagine à travers TOUS les anciens mails
// ============================================================================
export async function GET() {
  try {
    const gmail = await getGmailClient();

    // Cherche TOUS les mails des fournisseurs, même ceux déjà labelisés
    // On re-traite tout pour être sûr — les upserts sont idempotents
    const q = '(from:noreply@popina.com OR from:compta@foodflow.fr OR from:contact@milliet.paris OR from:commercial@lba-boissons.fr OR from:brasseriemapetitefrancaise@gmail.com)';

    // Récupérer tous les IDs en paginant
    ...
    do {
      ...
      for (const m of msgs) allMessageIds.push(m.id);
    ...
    const summary = {
      totalMails: allMessageIds.length,
      processed: 0,
    ...
    const TIME_LIMIT = 250_000; // 250s — marge avant le timeout de 300s
      ...
      try {
        ...
        if (source === 'popina') {
          ...
          await setDoc(doc(db, 'rapportsJournaliers', date), {
            date, menuNom, mois: moisStr,
            caTTC: rapport.caTTC, caHT: rapport.caHT,
            couverts: rapport.couverts, commandes: rapport.commandes,
            debutService: rapport.debutService, finService: rapport.finService,
            categories: rapport.categories,
            reductions: rapport.reductions, reductionsTotal: rapport.reductionsTotal,
            annulations: rapport.annulations, annulationsTotal: rapport.annulationsTotal,
            tvaNet: rapport.tvaNet, lieux: rapport.lieux, pourboires: rapport.pourboires,
          ...
          if (pdfs.length === 0) { summary.skipped++; summary.processed++; continue; }

          for (const pj of pdfs) {
            const buf = await downloadAttachment(gmail, msgId, pj.attachmentId);
            if (source === 'foodflow') {
        ...
        summary.errors.push(`msg ${msgId}: ${e.message}`);
        summary.processed++;
      }
    }
