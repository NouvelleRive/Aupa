...
const OLD_NAMES = ['Mayo'];
const NEW_NAME = 'Mayonnaise';

async function main() {
  console.log(`=== Fusion "Mayo" → "${NEW_NAME}" ===\n`);
  ...
  console.log('3) Suppression du doc "Mayo" dans ingredients...');
  const snap = await db.collection('ingredients').where('nom', '==', 'Mayo').get();
  if (snap.empty) {
